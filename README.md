...
| パッケージ | 役割 |
| --- | --- |
| [`@kuboon/how-to-a2hs`](./packages/add-to-home-screen) | ホーム画面に追加できるか判定し、できなければ標準ブラウザへ誘導、できる場合は機種別の手順を案内 |
| [`@kuboon/how-to-passkeys`](./packages/passkey-guide) | パスキーが使えるか判定し、使えなければ標準ブラウザへ誘導、使える場合はクロスデバイス等の仕組みを案内 |
...
```ts
import { detectDevice } from "@kuboon/how-to-a2hs";
...
2 パッケージは **JSR** と **GitHub Packages** で配布します（npm レジストリには公開しません）。

- JSR: `@kuboon/how-to-a2hs` / `@kuboon/how-to-passkeys`（`npx jsr add ...`）
- GitHub Packages: `@kuboon` スコープを `https://npm.pkg.github.com` に向けた `.npmrc` を用意して `npm i`

公開は `.github/workflows/publish.yml` で行います（GitHub Packages は `GITHUB_TOKEN`、JSR は OIDC trusted publishing）。各 README のインストール手順も参照してください。
...
pnpm --filter @kuboon/how-to-site dev   # デモサイトをローカル起動
```
